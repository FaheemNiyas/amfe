...
function DailyReport() {
    ...
    const downloadReport = () => {
        ...
        const tableRows = dailyReport.map(item => [
            ...
            item.description,
            item.type,
            item.type === 'Expense' ? `AED ${item.amount.toLocaleString('en-US')}` : '-',
            item.type === 'Income' ? `AED ${item.amount.toLocaleString('en-US')}` : '-',
        ]);
        ...
        // Append balance row
        const totalRow = ['', 'Total', '', `AED ${totalExpenseAmount.toLocaleString('en-US')}`, `AED ${totalIncomeAmount.toLocaleString('en-US')}`];
        const balanceRow = ['', 'Balance', '', '', `AED ${balance.toLocaleString('en-US')}`];
    ...
    return (
        <div className="container mt-3">
            ...
            <div className='card shadow col-md-4 p-3 mt-3'>
                <h5 className='text text-success '>Total Income: AED {totalIncome.toLocaleString('en-US')}</h5>
                <h5 className='text text-danger '>Total Expense: AED {totalExpense.toLocaleString('en-US')}</h5>
                <h5 className='text text-primary '>Balance: AED {balance.toLocaleString('en-US')}</h5>
            </div>
        </div>
    );
}
